...
class WeightPosterior:
    ...
    def log_marginal_likelihood(self, X_parents: np.ndarray, y_child: np.ndarray) -> float:
        ...
        Uses Woodbury identity to compute in O(n*p^2) instead of O(n^3):
        log|C| = n*log(sigma_eps^2) + log|I_p + (sigma_w^2/sigma_eps^2) X^T X|
        y^T C^{-1} y = (1/sigma_eps^2)(y^T y - sigma_w^2 * y^T X A^{-1} X^T y)
        where A = sigma_eps^2 I_p + sigma_w^2 X^T X
        """
        n, p = X_parents.shape

        XtX = X_parents.T @ X_parents  # p x p
        Xty = X_parents.T @ y_child     # p
        yty = y_child @ y_child          # scalar

        # A = sigma_eps^2 I + sigma_w^2 X^T X
        A = self.sigma_eps2 * np.eye(p) + self.sigma_w2 * XtX
        A += 1e-8 * np.eye(p)
        ...
        # log|C| via Woodbury: log|sigma_eps^2 I + sigma_w^2 X X^T|
        # = n*log(sigma_eps^2) + log|A| - p*log(sigma_eps^2)
        # = (n-p)*log(sigma_eps^2) + 2*sum(log(diag(L)))
        log_det_C = (n - p) * np.log(self.sigma_eps2) + 2.0 * np.sum(np.log(np.diag(L)))

        # y^T C^{-1} y via Woodbury
        v = np.linalg.solve(L, Xty)  # L^{-1} X^T y
        quad = (yty - self.sigma_w2 * (v @ v)) / self.sigma_eps2

        log_ml = -0.5 * (n * np.log(2 * np.pi) + log_det_C + quad)
        return float(log_ml)
